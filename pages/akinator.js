...
export default function Akinator() {
  ...
  const progress = Math.min((answers.length / 4) * 100, 100);
  ...
  async function handleOptionClick(selectedOption) {
    ...
    try {
      const res = await fetch('/api/akinator', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ answers: newAnswers, location: userLocation }),
      });
      const data = await res.json();
  ...
  async function retryLastRequest() {
    ...
    try {
      const res = await fetch('/api/akinator', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ answers, location: userLocation }),
      });
      const data = await res.json();
      ...
      <div style={{ background: 'white', borderRadius: 20, boxShadow: '0 20px 60px rgba(0,0,0,0.3)', maxWidth: 600, width: '100%', padding: 40, textAlign: 'center' }}>
        ...
        {phase === 'question' && (
          <>
            ...
            {aiError && (
              <div style={{ color: '#f44336', fontSize: '0.9em', marginBottom: 16 }}>
                {aiError}
                <button onClick={() => {
                  if (answers.length === 0) {
                    fetchInitialQuestion();
                ...
                }} style={{ marginLeft: 10, color: accentColor, background: 'none', border: `1px solid ${accentColor}`, borderRadius: 6, padding: '4px 10px', cursor: 'pointer', fontSize: '0.9em' }}>再試行</button>
              </div>
            )}
            <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
              {currentQuestion?.options.map((option, i) => (
                <button key={i} onClick={() => handleOptionClick(option)} disabled={aiLoading}
                  style={{ background: aiLoading ? '#ddd' : 'linear-gradient(135deg, #f093fb, #f5576c)', color: 'white', padding: '18px 25px', border: 'none', borderRadius: 10, cursor: aiLoading ? 'not-allowed' : 'pointer', fontWeight: 600, fontSize: '1em', transition: 'all 0.3s ease' }}
