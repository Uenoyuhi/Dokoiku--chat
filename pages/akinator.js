...
export default function Akinator() {
  ...
  async function callAkinator(answersToSend, loc) {
    ...
    if (data.type === 'question') {
      ...
      setPhase('result');
      if (loc) searchNearbyPlaces(data.destination.searchType, loc);
    }
  }
  ...
  async function handleOptionClick(selectedOption) {
    ...
    setAiError('');
    try {
      await callAkinator(newAnswers, userLocation);
    } catch {
      setAiError('AIの応答に失敗しました。もう一度試してください。');
    } finally {
      setAiLoading(false);
  ...
  async function handleRetry() {
    ...
    setAiError('');
    try {
      await callAkinator(answers, userLocation);
    } catch {
      setAiError('AIの応答に失敗しました。もう一度試してください。');
    } finally {
      setAiLoading(false);
        ...
        {phase === 'question' && (
          <>
            <div style={{ marginBottom: 30, minHeight: 60, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
              {aiLoading ? (
                <span style={{ display: 'inline-block', width: 32, height: 32, border: '4px solid #e0e0e0', borderTop: `4px solid ${accentColor}`, borderRadius: '50%', animation: 'spin 0.8s linear infinite' }} />
              ) : (
            ...
            {aiError && (
              <div style={{ marginBottom: 20 }}>
                <div style={{ color: '#f44336', fontSize: '0.9em', marginBottom: 10 }}>{aiError}</div>
                <button onClick={handleRetry} style={{ background: accentColor, color: 'white', padding: '10px 20px', border: 'none', borderRadius: 8, cursor: 'pointer', fontWeight: 600, fontSize: '0.9em' }}>再試行</button>
              </div>
            )}
